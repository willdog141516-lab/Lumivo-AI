...
test("chapter seek resets old playback before restoring the selected chapter", () => {
  const commands = [];
  const player = createStoryPlayer((command) => commands.push(command));

  player.load({ plan: nanjingTripPlan, timeline: nanjingStoryTimeline });
  commands.length = 0;
  player.seekChapter(2);

  assert.deepEqual(
    commands.map(({ type, chapterId }) => [type, chapterId]),
    [
      ["stage.clear", "chapter-intro"],
      ["poi.show", "chapter-day-2"],
    ],
  );
});
...
test("story playback runs at half speed while route animations stay aligned", () => {
  assert.equal(STORY_PLAYBACK_RATE, 0.5);
  assert.equal(toTimelineDelta(100), 50);
  assert.equal(toRealPlaybackDuration(1700), 3400);
  assert.equal(ROUTE_LINE_PLAYBACK_RATE, 1);
  assert.equal(toRealRouteDuration(1700), 3400);
});
