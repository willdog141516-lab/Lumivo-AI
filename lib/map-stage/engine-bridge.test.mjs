...
test("paused story rendering stops the continuous loop but renders the new state", () => {
  ...
  syncAnimationLoop(engine, false);

  assert.equal(engine.rendering.enableAnimationLoop, false);
  assert.equal(renderRequests, 1);
});

test("map rendering caps high-density displays at 1.5 device pixels", () => {
  assert.equal(limitMapPixelRatio(2), 1.5);
});
